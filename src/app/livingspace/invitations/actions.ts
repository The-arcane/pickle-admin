...
export async function inviteResidents(formData: FormData) {
  ...
  if (successCount > 0) {
    message += ` ${successCount} new invitation(s) were successfully sent.`;
  }
  ...
  if (successCount === 0 && residentsDataFromCsv.length > 0) {
    message = "No new users were invited. All users from the CSV were already part of this organization.";
  }


  // 7. Revalidate paths and return summary
  revalidatePath('/livingspace/invitations');
...
export async function removeResidence(formData: FormData) {
    ...
    if (error) {
        ...
        return { error: "Failed to remove invitation." };
    }
