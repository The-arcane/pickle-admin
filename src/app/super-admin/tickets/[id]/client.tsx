...
import { addTicketReply } from '@/app/arena/raise-ticket/actions';
            ...
            <CardContent className="pt-6 space-y-6">
                ...
                {initialMessages.map((msg) => (
                    <div key={msg.id} className={`flex items-start gap-3 ${msg.from === 'user' ? '' : 'justify-end'}`}>
                        ...
                        <div className={`p-3 rounded-lg max-w-lg ${msg.from === 'user' ? 'bg-muted' : 'bg-primary text-primary-foreground'}`}>
                        <p className="text-sm">{msg.message}</p>
                        <p className={`text-xs mt-1 ${msg.from === 'user' ? 'text-muted-foreground' : 'text-primary-foreground/70'}`}>{msg.time}</p>
                        </div>
                        {msg.from === 'support' && (
                        <Avatar className="h-8 w-8">
                            <AvatarFallback className={msg.isSuperAdmin ? 'bg-red-500 text-white' : ''}>{msg.userInitials}</AvatarFallback>
                        </Avatar>
                        )}
                    </div>
                ))}
