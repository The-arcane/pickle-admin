...
export default async function TicketConversationPage({ params }: { params: { id: string } }) {
    ...
    const { data: messages, error: messagesError } = await supabase
        ...
        .select('*, user:user_id!inner(id, name, profile_image_url, user_type)')
    ...
    const processedMessages = messages?.map(msg => ({
        ...
        from: msg.user?.id === ticket.user_id ? 'user' : 'support',
    ...
    <div className="space-y-6">
        <div className="flex items-center gap-3">
             <Button asChild variant="outline" size="icon" className="shrink-0">
                <Link href="/super-admin/tickets"><ChevronLeft className="h-4 w-4" /></Link>
            </Button>
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-muted-foreground font-mono">
                        ID: {ticket.id} | From: {ticket.organisation?.name} | Status: {ticket.status}
                    </p>
                </div>
            </div>
        </div>
