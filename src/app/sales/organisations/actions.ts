...
export async function addOrganization(formData: FormData) {
  ...
  try {
    // New organizations are created as inactive by default
    const insertData: { name: string; address: string; user_id: number; logo?: string, is_active: boolean } = {
        name,
        address,
  ...
  revalidatePath('/sales/organisations');
  return { success: true };
}
