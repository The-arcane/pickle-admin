...
export default function SalesDashboardPage() {
  ...
  useEffect(() => {
    async function getSalesDashboardData() {
      ...
      if(orgsError) {
        console.error("Error fetching inactive Living Spaces count", orgsError);
        setData({ inactiveOrganisations: 0 }); // Set a default on error
      } else {
        setData({
            inactiveOrganisations: count ?? 0,
        });
      }
      setLoading(false);
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {loading ? (
            Array.from({length: 1}).map((_, i) => (
                <Card key={i} className="p-4 flex flex-col justify-between h-full">
