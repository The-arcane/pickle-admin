...
export default async function ArenaEditCourtPage({ params }: { params: { id: string } }) {
  ...
  if (!userRecord) {
      return redirect('/login?type=arena&error=Could not determine your organization.');
  }
  ...
  if (!orgLink?.organisation_id) {
    return redirect('/arena/dashboard?error=Your%20account%20is%20not%20associated%20with%20an%20organization.');
  }
