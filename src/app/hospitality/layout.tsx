...
export default function HospitalityLayout({
  ...
  const getInitialData = useCallback(async () => {
    ...
    if (!profile) {
      redirect('/login?type=hospitality&error=Could%20not%20find%20user%20profile');
      return;
    }
    ...
    return (
        <div className="flex h-screen w-full items-center justify-center">
             <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-muted-foreground">Loading Hospitality Panel...</p>
            </div>
        </div>
    );
  }
      ...
      <div className="flex flex-col sm:pl-60 flex-1">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b bg-background px-6 shrink-0">
          ...
          <div className="ml-auto">
            <UserNav user={userProfile} basePath="/hospitality" />
          </div>
        </header>
