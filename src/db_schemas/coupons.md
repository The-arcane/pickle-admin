...
-- Table to log each time a coupon is used
CREATE TABLE IF NOT EXISTS public.coupon_usage (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    coupon_id BIGINT NOT NULL REFERENCES public.coupons(id) ON DELETE RESTRICT,
    user_id BIGINT NOT NULL REFERENCES public."user"(id) ON DELETE CASCADE,
    booking_id BIGINT REFERENCES public.bookings(id) ON DELETE SET NULL,
    event_booking_id BIGINT REFERENCES public.event_bookings(id) ON DELETE SET NULL,
    original_amount NUMERIC(10, 2) NOT NULL,
    discount_amount NUMERIC(10, 2) NOT NULL,
    final_amount NUMERIC(10, 2) NOT NULL,
    used_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    -- Can link to a transaction table if one exists
    -- transaction_id BIGINT REFERENCES public.transactions(id),
    CONSTRAINT chk_one_booking_ref CHECK (
        (booking_id IS NOT NULL AND event_booking_id IS NULL) OR
        (booking_id IS NULL AND event_booking_id IS NOT NULL) OR
        (booking_id IS NULL AND event_booking_id IS NULL) -- For general purpose coupons
    )
);

-- Trigger function to increment coupon usage count
...
-- Trigger that fires after a new record is inserted into coupon_usage
DROP TRIGGER IF EXISTS on_coupon_usage_insert ON public.coupon_usage;
...
CREATE POLICY "coupons_all_super_admins" ON public.coupons FOR ALL USING (EXISTS (SELECT 1 FROM public."user" u WHERE u.user_uuid = auth.uid() AND u.user_type = 3));
CREATE POLICY "coupon_courts_all_super_admins" ON public.coupon_courts FOR ALL USING (EXISTS (SELECT 1 FROM public."user" u WHERE u.user_uuid = auth.uid() AND u.user_type = 3));
CREATE POLICY "coupon_events_all_super_admins" ON public.coupon_events FOR ALL USING (EXISTS (SELECT 1 FROM public."user" u WHERE u.user_uuid = auth.uid() AND u.user_type = 3));
CREATE POLICY "coupon_usage_all_super_admins" ON public.coupon_usage FOR ALL USING (EXISTS (SELECT 1 FROM public."user" u WHERE u.user_uuid = auth.uid() AND u.user_type = 3));
...
-- Allow users to see their own coupon usage history
CREATE POLICY "coupon_usage_select_owner"
ON public.coupon_usage FOR SELECT
USING (EXISTS (SELECT 1 FROM public."user" u WHERE u.id = coupon_usage.user_id AND u.user_uuid = auth.uid()));

-- Org admins can view coupons and usage related to their org
CREATE POLICY "coupons_select_org_admin"
ON public.coupons FOR SELECT
USING (is_global = TRUE OR EXISTS (SELECT 1 FROM public.user_organisations uo JOIN public."user" u ON u.id = uo.user_id WHERE u.user_uuid = auth.uid() AND uo.organisation_id = coupons.organisation_id));

CREATE POLICY "coupon_usage_select_org_admin"
ON public.coupon_usage FOR SELECT
USING (EXISTS (
    SELECT 1
    FROM public.coupons c
    JOIN public.user_organisations uo ON c.organisation_id = uo.organisation_id
