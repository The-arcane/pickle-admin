...
| SA-PM-002 | Create New Organization (Happy Path) | 1. Navigate to an org page (e.g., `/super-admin/organisations`). 2. Click "Add Living Space". 3. Fill in Name, Address, and select an **unassigned** admin from the "Owner" dropdown. 4. Click "Save". | The organization is created and appears in the list. The selected admin is now assigned to it and their `user_type` is updated. | Pass |
| SA-PM-003 | Create Org with Assigned Admin | 1. Follow steps from SA-PM-002, but select an admin who **already owns** another organization. | An error message "This user is already an admin of another organization" is displayed. | Pass |
...
| SA-TICKET-003 | Reply to a Ticket | 1. Click "View" on a ticket. 2. Type a reply and send. | The reply appears in the conversation view. | Pass |
...
| EM-SCAN-002 | Scan Invalid QR Code | 1. Go to `/employee/scan`. 2. Scan a QR code that does not correspond to a booking. | A failure message "Booking not found" or similar appears. | Pass |
| EM-SCAN-003 | Check-in/Check-out Flow | 1. Go to `/employee/scan`. 2. Scan a valid booking QR. 3. Wait for success message. 4. Scan the *same QR code* again. | The first scan should show a "Check-in successful" message. The second scan should show a "Check-out successful" message. | Pass |
